<?php
	...
	if(check_wind_station($station_id) == true){
		...
		$output .= "<h3 style='color:#7D7D7D;'>Temp and Downfall</h3><br/><table id='wind_table' style='width:100%;' border='1px'><thead><th>Date and Time</th><th>   Downfall</th><th>Temp</th></thead>";
		...
		for($measurementIndex = 0; $measurementIndex < sizeof($stationdata); $measurementIndex++){
			...
			$row = "<tr><td>" . $time . "</td><td>" . $wndspd . "&nbsp; mm/w</td><td>" . $temp . "&nbsp;</td></tr>" . $row;
		}
